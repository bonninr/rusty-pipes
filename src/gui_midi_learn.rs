...
pub fn draw_midi_learn_modal(
            ...
            if *time > learn_state.last_interaction {
                ...
                match &learn_state.target {
                    LearnTarget::Stop(idx) => {
                        state.midi_control_map.learn_stop(*idx, target_internal, event_clone.clone(), is_enable);
                    }
                ...
                let action_text = if is_enable { t!("midi_learn.action_enable") } else { t!("midi_learn.action_disable") };
                state.add_midi_log(
    ...
    let window_title = t!("midi_learn.window_title_fmt", name = learn_state.target_name);
            ...
            match target {
                LearnTarget::Stop(idx) => {
                    ...
                    let control_map = {
                        ...
                        state.midi_control_map.stops.get(&idx).cloned().unwrap_or_default()
                    };
...
fn draw_tremulant_row(
    ...
    egui::Grid::new("trem_learn_grid")
        ...
        .show(ui, |ui| {
            ...
            if ui.add(egui::Button::new(enable_txt).selected(learn_state.learning_slot == Some((0, true)))).clicked() {
            ...
            if ui.add(egui::Button::new(disable_txt).selected(learn_state.learning_slot == Some((0, false)))).clicked() {
                learn_state.last_interaction = Instant::now();
        ...
        egui::Grid::new("midi_learn_grid")
            ...
            .show(ui, |ui| {
                ...
                for channel in 0..16u8 {
                    ...
                    if ui.add(egui::Button::new(enable_btn_text).selected(learn_state.learning_slot == Some((channel, true)))).clicked() {
                        learn_state.last_interaction = Instant::now();
                    ...
                    if ui.add(egui::Button::new(disable_btn_text).selected(learn_state.learning_slot == Some((channel, false)))).clicked() {
                        learn_state.last_interaction = Instant::now();
