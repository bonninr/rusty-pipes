    ...
    pub fn handle_input(&mut self, key: KeyCode, app_state: &Arc<Mutex<AppState>>) -> bool {
        ...
        match key {
            ...
            KeyCode::Enter => {
                ...
                let effective_row = if let LearnTarget::Tremulant(_) = self.target { 0 } else { self.row_idx };
...
pub fn draw_midi_learn_modal(frame: &mut Frame, tui_state: &MidiLearnTuiState, app_state: &AppState) {
    ...
    let title = format!(" {} ", t!("midi_learn.window_title_fmt", name = tui_state.target_name));
    ...
    let rows: Vec<Row> = match &tui_state.target {
        LearnTarget::Stop(idx) => {
            let control_map = app_state.midi_control_map.stops.get(idx).cloned().unwrap_or_default();
            (0..16u8).map(|channel| {
        ...
        LearnTarget::Tremulant(id) => {
            let control = app_state.midi_control_map.tremulants.get(id).cloned().unwrap_or_default();
