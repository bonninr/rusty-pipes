...
impl MidiMappingWindow {
    ...
    pub fn show(&mut self, ctx: &egui::Context, devices: &mut Vec<MidiDeviceConfig>) {
        ...
        let window_title = format!("MIDI Mapping: {}", device.name);
        ...
        egui::Window::new(window_title)
            ...
            .show(ctx, |ui| {
                ...
                ui.label("Configure how MIDI channels from this device map to the organ's internal channels.");
                ui.add_space(10.0);

                // --- Mode Selection ---
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new("Mapping Mode:").strong());
                    ui.radio_value(&mut device.mapping_mode, MidiMappingMode::Simple, "Simple");
                    ui.radio_value(&mut device.mapping_mode, MidiMappingMode::Complex, "Complex");
                });
    ...
    fn render_simple_mode(ui: &mut egui::Ui, device: &mut MidiDeviceConfig) {
        ui.vertical_centered(|ui| {
            ui.add_space(10.0);
            ui.label("In Simple Mode, all incoming MIDI messages from this device");
            ui.label("are routed to a single target channel on the organ.");
            ui.add_space(10.0);

            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Target Channel:").strong());
                
                // ComboBox for 1-16 (stored as 0-15)
                egui::ComboBox::from_id_salt("simple_target_selector")
                    .selected_text(format!("Channel {}", device.simple_target_channel + 1))
                    .show_ui(ui, |ui| {
                        for ch in 0..16 {
                            ui.selectable_value(
    ...
    fn render_complex_mode(ui: &mut egui::Ui, device: &mut MidiDeviceConfig) {
        ui.label("Map each specific input channel to a specific target channel.");
        ui.add_space(5.0);
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                egui::Grid::new("complex_map_grid")
                    ...
                    .show(ui, |ui| {
                        // Header
                        ui.label(egui::RichText::new("Input Channel").strong().underline());
                        ui.label(egui::RichText::new("Target Channel").strong().underline());
                        ui.end_row();

                        // Rows 1-16
                        for i in 0..16 {
                            ui.label(format!("Input Channel {}", i + 1));
